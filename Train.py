...
class ImageSequence(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            ...
            # We have a little hack here - we save captchas as TEXTAnum.png if there is more than one captcha with the text "TEXT"
            # So the real label should have the "Anum" stripped out.

            random_image_label = random_image_label.split('A')[0]
            ...
            if(len(random_image_label) < self.captcha_length):
                random_image_label = random_image_label.ljust(self.captcha_length, 'A')
            for d in decode:
                random_image_label = random_image_label.replace(d, special_symbols[decode.index(d)])
            # print("image label: " + random_image_label)
...
def main():
    ...
    # with tf.device('/device:XLA_CPU:0'):
        ...
        open(args.output_model_name + 'tflite', "wb").write(model)
